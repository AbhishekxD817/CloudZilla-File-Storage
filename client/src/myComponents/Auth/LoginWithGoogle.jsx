...
import { authWithGoogle } from '../../store/actions/authActions.js'
...
const LoginWithGoogle = () => {
    ...
    const handleGoogleResponse = async (authResponse) => {
        try {
            if (authResponse['code']) {
                const response = await dispatch(authWithGoogle(authResponse['code']));
                if (response.type == 'authWithGoogle/fulfilled') {
                ...
                toast.error("google auth code error");
                return;
            }

        } catch (error) {
            const { message = "Error while signin in with google" } = error;
            toast.error(message);
            return;
        }
    }
    ...
    return (
        <>
            <Button
                onClick={googleLogin}
                className=" py-2 px-16 flex items-center justify-center space-x-4 bg-white border border-gray-300 rounded-lg shadow-sm hover:shadow-md "
            >
                <img
